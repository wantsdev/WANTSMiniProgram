...
var WNTSToken = require('./WNTSToken');
var WNTSUserInfo = require('./WNTSUserInfo');
      ...
      wx.getUserInfo({
        success: function (userResult) {
          callback(null, {
            ...
            user_info_json_string: userResult.rawData,
            device_id: ""
  ...
  var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
    ...
    wx.request({
      ...
      success: function (result) {
        console.log(options.loginUrl, options.method, options.data, result);
        var data = result.data;
        if (result.statusCode == 200 && data.token) {
          var res = data;
          if (res.user) {
            console.log(res);
            WNTSToken.set(res.token);
            WNTSUserInfo.set(res.user);
            options.success(res.user);
          } else {
            var errorMessage = '登录失败(' + data.code + ')：' + (data.msg || '未知错误');
            var noTokenError = new LoginError(constants.ERR_LOGIN_TOKEN_NOT_RECEIVED, errorMessage);
            options.fail(noTokenError);
          }
        } else {
          var noTokenError = new LoginError(constants.ERR_LOGIN_TOKEN_NOT_RECEIVED, JSON.stringify(data));
          options.fail(noTokenError);
        }
        // return;
        //   var data = result.data;
